  ...
  db.serialize(() => {
    ...
    db.get('SELECT COUNT(*) as count FROM interfaces', (err, row: any) => {
      if (!err && row && row.count === 0) {
        ...
        db.run('INSERT INTO interfaces (name, physical_interface, ip_address, netmask) VALUES (?, ?, ?, ?)', ['WAN', 'eth0', '', '']);
        db.run('INSERT INTO interfaces (name, physical_interface, ip_address, netmask) VALUES (?, ?, ?, ?)', ['LAN', 'eth1', '', '']);
      }
    });
  });
}
