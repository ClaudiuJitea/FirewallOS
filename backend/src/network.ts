...
function getIpv4InterfaceRows(): Array<{ iface: string; addr: string }> {
    try {
...
export async function getInterfaceNameMap(): Promise<Record<string, string>> {
    const interfaces = await allQuery('SELECT name, physical_interface FROM interfaces');
    const interfaceWithIp = await allQuery('SELECT name, ip_address FROM interfaces');
    ...
    for (const row of interfaces) {
        ...
        if (logical && physical) map[logical] = physical;
    }
    ...
    for (const row of interfaceWithIp) {
        ...
        if (!logical || !ip) continue;
        const hit = rows.find((r) => r.addr.startsWith(`${ip}/`));
        if (hit) map[logical] = hit.iface;
    }
    return map;
}

export async function resolveRuntimeInterface(input: string): Promise<string> {
...
export async function applyAllInterfaceConfigs(): Promise<void> {
    const interfaces = await allQuery('SELECT * FROM interfaces ORDER BY id ASC');
    for (const iface of interfaces) {
        await applyInterfaceConfig(iface);
    }
}

export async function applyInterfaceConfig(iface: any): Promise<void> {
