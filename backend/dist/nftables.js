...
exports.initNftables = initNftables;
exports.applyAllRules = applyAllRules;
exports.applyRoutes = applyRoutes;
exports.getCurrentNftRuleset = getCurrentNftRuleset;
 ...
 */
function execNft(command) {
    try {
        const result = (0, child_process_1.execSync)(`nft ${command}`, {
            encoding: 'utf-8',
            timeout: 10000,
        });
        return result;
 ...
 */
function buildIpMatch(field, value) {
    if (!value || value === 'ANY')
        return '';
 ...
 */
function buildPortMatch(proto, portStr) {
    if (!portStr || portStr === 'ANY' || !proto)
        return '';
 ...
 */
function buildFilterRules(rules, interfaceMap) {
    const commands = [];
    // Safety rules first: always allow established/related connections
    commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} input ct state established,related accept`);
    commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} forward ct state established,related accept`);
    // Safety: always allow management ports (80=frontend, 3000=backend API)
    commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} input tcp dport { 80, 3000 } accept`);
    // Safety: allow loopback
    commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} input iif lo accept`);
    for (const rule of rules) {
        ...
        if (dstMatch)
            parts.push(dstMatch);
        if (portMatch)
            parts.push(portMatch);
        parts.push(verdict);
        const ruleExpr = parts.join(' ');
        // Apply to input and forward chains
        commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} input ${ruleExpr} comment "${rule.name || 'rule-' + rule.id}"`);
        commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} forward ${ruleExpr} comment "${rule.name || 'rule-' + rule.id}"`);
    }
    return commands;
}
/**
 ...
 */
function buildNatRules(natRules, interfaceMap) {
    const commands = [];
    for (const rule of natRules) {
        ...
        if (type === 'DNAT') {
            ...
            commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} prerouting ${parts.join(' ')}`);
        }
        else if (type === 'SNAT') {
            // SNAT: postrouting chain
            const parts = [];
            if (resolvedIface && resolvedIface !== 'ANY')
                parts.push(`oifname "${resolvedIface}"`);
            if (proto)
                parts.push(`meta l4proto ${proto}`);
            if (srcMatch)
                parts.push(srcMatch);
            ...
            parts.push(`comment "${rule.name || 'nat-' + rule.id}"`);
            commands.push(`add rule ${TABLE_FAMILY} ${TABLE_NAME} postrouting ${parts.join(' ')}`);
        }
    }
    return commands;
}
 ...
 */
function buildDnsRedirectRules(interfaces, interfaceMap) {
    const commands = [];
    const seen = new Set();
    for (const iface of interfaces) {
        ...
        if (!runtimeIface || runtimeIface.toUpperCase() === 'ANY')
            continue;
        if (seen.has(runtimeIface))
            continue;
...
async function applyAllRules() {
    try {
        ...
        const interfaces = await (0, db_1.allQuery)('SELECT * FROM interfaces WHERE status = 1');
        const interfaceMap = await (0, network_1.getInterfaceNameMap)();
