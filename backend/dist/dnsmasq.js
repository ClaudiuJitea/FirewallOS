 ...
 */
function initDnsmasq() {
    console.log('[dnsmasq] Initializing dnsmasq...');
    // Ensure config directory exists
    try {
        if (!fs_1.default.existsSync(DNSMASQ_CONF_DIR)) {
            fs_1.default.mkdirSync(DNSMASQ_CONF_DIR, { recursive: true });
        }
    }
    catch (err) {
    ...
    try {
        fs_1.default.writeFileSync(DNSMASQ_BASE_CONF, baseConfig, 'utf-8');
    ...
    try {
        fs_1.default.writeFileSync(DNSMASQ_CONF_FILE, '# Firewall Manager DNS rules\n', 'utf-8');
    }
    catch (err) {
...
function getDnsmasqStatus() {
    ...
    // Read current config
    try {
        config = fs_1.default.readFileSync(DNSMASQ_CONF_FILE, 'utf-8');
    }
    catch {
 ...
 */
async function applyDhcpConfig() {
    try {
        console.log('[dnsmasq] Applying DHCP configuration from database...');
        const pools = await (0, db_1.allQuery)('SELECT * FROM dhcp_pools WHERE status = 1');
        const staticLeases = await (0, db_1.allQuery)('SELECT * FROM dhcp_static_leases WHERE status = 1');
        ...
        for (const lease of staticLeases) {
            ...
            if (lease.hostname)
                parts.push(lease.hostname);
            configLines.push(`# Static: ${lease.description || lease.hostname || lease.mac_address}`);
            configLines.push(`dhcp-host=${parts.join(',')}`);
        }
        configLines.push('');
        // Write config
        fs_1.default.writeFileSync(DHCP_CONF_FILE, configLines.join('\n'), 'utf-8');
        // Ensure lease file directory exists and migrate legacy lease file once.
        try {
            const leaseDir = path_1.default.dirname(DHCP_LEASE_FILE);
            if (!fs_1.default.existsSync(leaseDir)) {
                fs_1.default.mkdirSync(leaseDir, { recursive: true });
            }
            if (!fs_1.default.existsSync(DHCP_LEASE_FILE)) {
                if (fs_1.default.existsSync(LEGACY_DHCP_LEASE_FILE)) {
                    fs_1.default.copyFileSync(LEGACY_DHCP_LEASE_FILE, DHCP_LEASE_FILE);
                }
                else {
                    fs_1.default.writeFileSync(DHCP_LEASE_FILE, '', 'utf-8');
                }
            }
        }
        catch (e) {
...
function getDhcpLeases() {
    try {
        if (!fs_1.default.existsSync(DHCP_LEASE_FILE))
            return [];
        const content = fs_1.default.readFileSync(DHCP_LEASE_FILE, 'utf-8').trim();
        if (!content)
            return [];
        return content
            .split('\n')
