...
router.get('/rules', async (req, res) => {
    try {
        const rules = await (0, db_1.allQuery)('SELECT * FROM dns_rules ORDER BY id DESC');
        res.json(rules);
    }
    catch (error) {
...
router.post('/rules', async (req, res) => {
    ...
    try {
        const result = await (0, db_1.runQuery)('INSERT INTO dns_rules (domain, action, status) VALUES (?, ?, ?)', [domain, action, status ?? 1]);
        res.status(201).json({ id: result.lastInsertRowid });
        (0, dnsmasq_1.applyDnsRules)().catch(err => console.error('[dnsmasq] Sync after DNS rule create failed:', err));
    }
    catch (error) {
...
router.put('/rules/:id', async (req, res) => {
    ...
    const { domain, action, status } = req.body;
    try {
        await (0, db_1.runQuery)('UPDATE dns_rules SET domain = ?, action = ?, status = ? WHERE id = ?', [domain, action, status === false ? 0 : 1, id]);
        res.json({ success: true });
        (0, dnsmasq_1.applyDnsRules)().catch(err => console.error('[dnsmasq] Sync after DNS rule update failed:', err));
    }
    catch (error) {
...
router.delete('/rules/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await (0, db_1.runQuery)('DELETE FROM dns_rules WHERE id = ?', [id]);
        res.json({ success: true });
        (0, dnsmasq_1.applyDnsRules)().catch(err => console.error('[dnsmasq] Sync after DNS rule delete failed:', err));
    }
    catch (error) {
...
router.get('/country-rules', async (req, res) => {
    try {
        const rules = await (0, db_1.allQuery)('SELECT * FROM country_rules ORDER BY id DESC');
        res.json(rules);
    }
    catch (error) {
...
function fetchGeoIp(ip) {
    return new Promise((resolve) => {
        http_1.default.get(`http://ip-api.com/json/${ip}`, (res) => {
            let data = '';
...
router.post('/test', async (req, res) => {
    ...
    try {
        ...
        try {
            const addresses = await promises_1.default.resolve4(domain);
        ...
        if (!matchFound && countryCode !== 'Unknown') {
            const countryRules = await (0, db_1.allQuery)('SELECT * FROM country_rules WHERE status = 1 AND country_code = ?', [countryCode]);
            if (countryRules.length > 0) {
                action = countryRules[0].action;
            }
        }
        // 5. Log the query
        const result = await (0, db_1.runQuery)('INSERT INTO dns_logs (domain, ip_address, country_code, latitude, longitude, action) VALUES (?, ?, ?, ?, ?, ?)', [domain, ipAddress, countryCode, lat, lon, action]);
        res.json({
